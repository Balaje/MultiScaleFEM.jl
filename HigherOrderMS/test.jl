...
basis_cache = ms_basis_cache!(matcache, D, nf, (q,p), basis_vec_ms, patch_indices_to_global_indices)
...
# Compute MS bases
basis_vec_ms = compute_ms_bases!(basis_cache, p, l)
# Need to rewrite the assembly for this. Basically, extract and store the bases function indices for each elements
basis_vec_ms_el = convert_to_cell_wise(basis_vec_ms, nc); 


mat_contribs = BroadcastVector(mat_contribs!, mat_contrib_cache, Ds, ∇Φs, ∇Φs, jacobian_exp*(-1));
...
vec_contribs = BroadcastVector(vec_contribs!, vec_contrib_cache, fs, Φs, jacobian_exp);
