...
function _local_matrix!(Me::Array{Float64}, basis::Tuple{Vector{Float64},Vector{Float64}}, 
  h::Float64, fespace::Tuple{Int64,Int64})
  q,p = fespace
  basis_1, basis_2 = basis
  # Loop over the local matrices
  for j=1:p+1, i=1:q+1
    @inbounds Me[i,j] += basis_1[i]*basis_2[j]*(0.5*h)
  end
end
function _local_vector!(Fe::Array{Float64}, basis::Vector{Float64}, h::Float64, fespace::Int64)
  p = fespace
  for i=1:p+1
    @inbounds Fe[i] += basis[i]*(0.5*h)
  end
end
