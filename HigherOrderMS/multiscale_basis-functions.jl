...
function compute_ms_correctors(fspace::FineScaleSpace, D::Function, p::Int64, nc::Int64, l::Int64,
  patch_indices_to_global_indices::Vector{AbstractVector{Int64}}, bvals::Vector{Float64})
  nf = fspace.nf
  q = fspace.q
  basis_vec_corr = spzeros(Float64, q*nf+1, (p+1)*2)
  K, L, Λ = get_saddle_point_problem(fspace, D, p, nc)
  f1 = zeros(Float64,size(K,1))
  index = 1
  for t in [1,nc]
    fullnodes = patch_indices_to_global_indices[t]    
    bn = [fullnodes[1], fullnodes[end]]
    bv = bvals[bn]
    freenodes = setdiff(fullnodes, bn)
    start = max(1, t-l)
    last = min(nc, t+l)
    gn = start*(p+1)-p:last*(p+1)    
    stima_el = K[freenodes,freenodes]
    lmat_el = L[freenodes,gn]
    loadvec_el = (f1 - K[:,bn]*bv)
    for _=1:p+1
      fvecs_el = [loadvec_el[freenodes]; Λ[gn, index]]
      lhs = [stima_el lmat_el; (lmat_el)' spzeros(Float64, length(gn), length(gn))]
      rhs = fvecs_el           
      sol = lhs\rhs                 
      basis_vec_corr[freenodes,index] = sol[1:length(freenodes)]
      basis_vec_corr[bn, index] = bv
      index += 1   
    end    
  end
  basis_vec_corr
