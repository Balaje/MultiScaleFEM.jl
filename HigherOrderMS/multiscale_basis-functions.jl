...
module MultiScaleBases
  ...
  function compute_ms_bases!(cache, p::Int64)
    mats, fvecs, bases_data = cache
    f1, f2 = fvecs
    stima, lmat = mats
    nds_elem, basis_vec_ms, patch_to_fine_scale = bases_data
    ...
    for t=1:nc
      ...
      lmat_el = lmat[gn[fn], :]
      for tt=1:p+1
        fvecs_el = vcat(f1[gn[fn],index], f2[:,index])
  ...
  function ms_basis_cache!(matcache, nds::Tuple{AbstractVector{Float64}, AbstractVector{Float64}}, 
    ...
    (stima, lmat), (zeros(Float64, q*nf+1, nc*(p+1)), fvecs), (nds_coarse[elem_coarse], basis_vec_ms, patch_to_fine_scale)
  end
end