...
for i in 1:lastindex(N)
  ...
  local Vₕᴹˢ = MultiScale(Ω, A, (q,p), l, [1,n*p+n-p]; Nfine=nₚ, qorder=qorder) # Multiscale space
...
plt1 = plot((1 ./N), (H¹errs), yaxis=:log10, xaxis=:log10, label="H¹ Error", lw=2, lc=:blue)
plt2 = plot((1 ./N), (L²errs), yaxis=:log10, xaxis=:log10, label="L² Error", lw=2, lc=:red)
...
plot!(plt2, (1 ./N), (1 ./N).^2, yaxis=:log10, xaxis=:log10, label="order 2", ls=:dash, lc=:black)
