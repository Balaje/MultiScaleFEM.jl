...
function preallocate_matrices(domain::Tuple{Float64,Float64}, nc::Int64, nf::Int64, l::Int64, fespaces::Tuple{Int64,Int64},
  ...
  basis_elem_multiscale = Vector{SparseMatrixCSC{Float64,Int64}}(undef,nc)
  ...
  multiscale_vector = zeros(Float64, (p+1)*nc)  
    ...
    basis_elem_multiscale[i] = spzeros(Float64, q*nf+1, npatch*(p+1))        
  end
  ...
  (Ds, fs, Φs, ∇Φs, jacobian_exp, nc_elem, p_elem, l_elem) 
end
