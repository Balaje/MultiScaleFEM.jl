...
function preallocate_matrices(domain::Tuple{Float64,Float64}, 
  ...
  for i=1:nc
    ...
    basis_vec_patch[i] = zeros(Float64, nfᵢ*q+1, p+1)   
  ...
  ipcache = zeros.(Float64,length.(patch_indices_to_global_indices))
  (nds_coarse, elem_coarse, nds_fine, elem_fine, assem_H¹H¹), 
  (nds_fineₛ, elem_fineₛ), (nds_patchₛ, patch_elemₛ, patch_indices_to_global_indices, ipcache), 
  basis_vec_patch, 
  (sKeₛ, sLeₛ, sFeₛ, sLVeₛ), (assem_H¹H¹ₛ, assem_H¹L²ₛ), (sKms, sFms)
end