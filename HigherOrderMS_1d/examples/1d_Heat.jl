...
let         
  ...
  t = 0.0
  # Starting BDF steps (1...k-1) 
  @showprogress desc="Compute Multiscale Solution 1 to $(BDF-1)" for i=1:BDF-1
    dlcache = get_dl_cache(i)
    cache = dlcache, fcache
  ...
  dlcache = get_dl_cache(BDF)
  cache = dlcache, fcache
  @showprogress desc="Compute Multiscale Solution $BDF to $ntime" for i=BDF:ntime
...
println(" ")
println("$nf \t $nc \t $p \t $l \t 0 \t $L²Error \t $H¹Error")